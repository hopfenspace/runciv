...
/** {@link React.Context Context} to access {@link AccountResponse user information} */
const USER_CONTEXT = React.createContext<UserContext>({
    user: { username: "", displayName: "", uuid: "" },
});
...
type UserProviderState = {
    user: AccountResponse | "unauthenticated" | null;
};
...
export class UserProvider extends React.Component<UserProviderProps, UserProviderState> {
    ...
    fetchUser() {
        if (this.state.user == null)
            Api.accounts.getMe().then((result) =>
                result.match(
